...
The main thresholding command in ImageJ is **Image → Adjust → Threshold...**, with the shortcut `Shift+T`.
...
These options are controlled using a combination of the threshold sliders and the **Dark background** checkbox.
There is also a drop-down menu allowing you to select from a list of automated thresholding methods.

![](assets/2.3/imagej-thresholds-dialog.png)
...
After choosing suitable thresholds, pressing **Apply** produces a binary image.
...
> To complicate matters, ImageJ also permits _either_ of these to represent the foreground, with the choice hidden away under **Process → Binary → Options...**, and 0 taken to be 'black' and 255 'white'.
> Personally, I prefer for white to represent the foreground (i.e.
> the interesting things we have detected), and so I will assume that the **Black background** option has been checked.
>
> Nevertheless, you should be aware that this convention is not adopted universally.
> Furthermore, if you choose **Invert LUT** then the colors are flipped anyway – so yet more confusion arises.
...
**Image → Adjust → Auto Threshold...** helps with this, by providing an option to try *all* of the methods.

![](assets/2.3/imagej-thresholds-all-dialog.png)

Applying this to ImageJ's famous *blobs.gif* reveals that not all methods work equally well:

![](assets/2.3/imagej-thresholds-all-blobs.png)
...
* Click on an object with the *Wand tool* ![](assets/2.3/wand.png) -- this is good for interactively choosing one connected region.
* **Edit → Selection → Create Selection** -- this makes a single ROI containing all the foreground pixels.
Disconnected regions can be separated by adding the ROI to the *ROI Manager* and choosing **More >> Split**.
* **Analyze → Analyze Particles...** -- this detects and measures all the foreground regions as individual objects, optionally filtering out objects based on shape or area.

> **📌 Creating ROIs without applying a threshold**
>
> The *Wand tool* ![](assets/2.3/wand.png), **Edit → Selection → Create Selection** & **Analyze → Analyze Particles...** can also be used when a threshold is being previewed on an image, but it has not yet been converted to binary.
...
I like the fact that **Edit → Selection → Create Selection** provides a very quick way to convert a thresholded region to a single ROI.
I most often use it in combination with **Edit → Selection → Restore Selection** to visualize what has been detected on top of another image.
I might not necessarily *use* the single ROI for measurements in the end, but it can be very good for orientation.

When you want to measure multiple objects quickly, **Analyze → Analyze Particles...** is arguably the most automated and versatile option.
Its various options also make it possible to ignore regions that are particularly small or large, straight or round (using a **Circularity** metric).
It can output summary results and add ROIs for each region to the *ROI Manager*.

![](assets/2.3/imagej-thresholds-analyze-dialog.png)
...
>
> It's possible to do this by changing the **Redirect to:** option under **Analyze → Set Measurements...**, which redirects measurements to be made on a specific image.
> Setting a redirection image before running **Analyze → Analyze Particles...** means that the ROI 'particles' can be detected on one image and measured on a different image
>
> I don't usually recommend this, since I imagine I might forget to reset the **Redirect to:** option when I am done.
> I'd rather add my ROIs to the *ROI Manager* and transfer them that way.

> **📌 Caution with holey ROIs**
>
> Historically, there has been a problem with my preferred approach to use the *ROI Manager* output from **Analyze → Analyze Particles...**.
>
> It seems that **Analyze Particles...** handles holes correctly when making measurements by itself, but by default it would generate ROIs that lacked holes.
...
>
>
> ![Different results when using **Analyze → Analyze Particles...** directly, and then subsequently measuring the ROI that it generated. The image is binary, so the mean value should be 255 if only the above-threshold pixels are measured.](assets/2.3/imagej-thresholds-analyze-holes.png)
>
> *Figure —* Different results when using **Analyze → Analyze Particles...** directly, and then subsequently measuring the ROI that it generated. The image is binary, so the mean value should be 255 if only the above-threshold pixels are measured.
>
>
> **However**, there is a solution -- which I only learned when writing about it here.
> Since ImageJ v1.53g, there is a **Composite ROIs** checkbox when using **Analyze Particles...**.
> If this is turned **on**, then ROIs with holes are generated as expected.
>
>
> ![Fixing the problem with holey regions by using **Composite ROIs**.](assets/2.3/imagej-thresholds-analyze-composite.png)
>
> *Figure —* Fixing the problem with holey regions by using **Composite ROIs**.
>
>
...
**Analyze → Analyze Particles...** can generate labeled images by using the **Show: Count Masks** option.
...
With a suitably colorful LUT (often **Image → Lookup Tables → Glasbey**), this can create a helpful and cheerful display of objects (the figure).

![Creating a labeled image with **Analyze → Analyze Particles**.](assets/2.3/imagej-thresholds-analyze-labels.png)

*Figure —* Creating a labeled image with **Analyze → Analyze Particles**.
...
> * the *Wand tool* ![](assets/2.3/wand.png)
>
> **Note:** You can investigate this by applying the commands/tools to a binary image that has some diagonally connected pixels.
> One way to do this is by thresholding a suitable image, another is to use the *Brush tool* ![](assets/2.3/brush.png) to draw your own pixels in the binary image.
>
...
At the time of writing, **Analyze → Analyze Particles...** uses 8-connectivity.
...
![](assets/2.3/imagej-thresholds-wand-dialog.png)

</details>

## Local thresholding

A few methods to perform local thresholding can be found under **Image → Adjust → Auto Local Threshold**.
The dialog again provides the option to try all of them.

![](assets/2.3/imagej-thresholds-local-dialog.png)
...
Explore several automated methods of thresholding the different channels of **File → Open samples → HeLa Cells**, using both local and global automated thresholds, to draw your own conclusions about which methods you might prefer for different images.
...
The first issue appears when you click **Apply** in the **Image → Adjust → Threshold...** dialog box for a 32-bit image.
This leads to a prompt asking whether to **Set Background Pixels to NaN**.
...
The same type of conversion is used for 16-bit images -- _unless_ you use the **Image → Adjust → Auto Threshold** command, which can use a full 16-bit histogram with 65536 bins.
...
Personally, I find this confusing; I don't like the brighness/contrast impacting thresholds, and my poor brain even struggles to process the negative in the **Don't reset range** to get an intuitive feeling for what effect the option will really have.
...
> **📝 Practical**
> Explore setting thresholds with and without **Don't reset range** selected for the image **File → Open Samples → M51 Galaxy (16-bits)**, and pressing the **Auto** button to determine a threshold.
>
> Keep **Image → Adjust → Brightness/Contrast...** open as you explore, so you can make adjustments before calculating the thresholds.
>
>
> ![](assets/2.3/imagej-m51.png)
>
>
> [▶ Launch ImageJ.JS](https://ij.imjoy.io/?run=https://gist.github.com/petebankhead/c5d2075777c458c70c69a1f2c8b37391)
...
![Thresholding a 16-bit image with and without 'Don't reset range' selected.](assets/2.3/gen_2.3_29.png)

*Figure —* Thresholding a 16-bit image with and without 'Don't reset range' selected.
...
>
> To explore this, you can use the extreme example of *cell_outlier.tif* along with the **Image → Adjust → Threshold...** command.
>
> How could you (manually) reduce the impact of any problems you find?
>
> **Note:** **Analyze → Histogram** lets you investigate the image histogram with different numbers of bins -- but any changes you make here will not be reflected in the histogram actually used for thresholding.
>
> [▶ Launch ImageJ.JS](https://ij.imjoy.io?open=https://github.com/bioimagebook/practical-data/blob/main/images/cell_outlier.tif)
...
1. Adjust the brightness/contrast sliders and ensure that **Don't reset range** **is** selected. This is the benefit of the option existing.
2. Convert the image to 8-bit manually yourself. This allows you to effectively choose the range of the histogram bins (using **Brightness/Contrast...**; see Types & bit-depths) Since the threshold is made using 256 bins, you are not really losing any information that was not going to be lost anyway.
