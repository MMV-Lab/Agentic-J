...
The Difference of Gaussians macro developed in chap_macro_intro was useful, but quite simple.
...
![Example input image](assets/4.3.2/macros_simulated_orig.png)

*Figure —* Example input image

![Example output image](assets/4.3.2/macros_simulated.png)

*Figure —* Example output image

* Ensure the starting image is 32-bit. 
* Run **Process → Filters → Gaussian Blur...** using a sigma value of 2 to simulate the convolution with the PSF. 
...
So we can simply add a constant to this image using **Add...**.
...
Because adjusting the exposure works like a multiplication, we can use the **Process → Math → Multiply...** command. Set it to a 'default' value of 1 for now. 
...
At the time of writing, there's no built-in command in ImageJ or Fiji to simulate Poisson noise but we can install *RandomJ* using the instructions at https://imagescience.org/meijering/software/randomj/. Then add noise by calling **Plugins → RandomJ → RandomJ Poisson**, making sure to set the **Insertion:** value to **Modulatory**. The **Mean** value will be ignored, so its setting does not matter.
...
* Clip any negative values, by running **Process → Math → Min...** and setting the value to 0. 
* Clip any positive values that exceed the bit-depth, by running **Process → Math → Max...**. To assume an a 8-bit image, set the value to 255. 

Now is a good time to clean up the code by removing any unnecessary lines, adding suitable comments, and bringing any interesting variables up to the top of the macro so that they can be easily modified later (as in chap_macro_dog).
...
Normalization is now possible using **Subtract** and **Divide** commands, and adjusting their values. In the end this gives us 
...
This could be done using the **Image → Transform → Bin** command, with a **shrink factor** of 2 and the **Sum** bin method.
...
There are a few ways to do this: we can convert the image using **Image → Type →** commands, though then we need to be careful about whether there will be any scaling applied.
