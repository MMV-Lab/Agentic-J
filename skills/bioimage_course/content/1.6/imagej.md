...
As described in chap_imagej_pixels, the general approach to open an image is to drag the file on top of the ImageJ toolbar and hope for the best.
...
![](assets/1.6/imagej-files-import-sequence.png)

This assumes that *all* the images in the folder should be opened.
If you need more control, then **File → Import → Image Sequence...** can help.
...
![](assets/1.6/imagej-files-import-sequence-full.png)

> **📌 Virtual stacks for large images**
>
> Both the dialogs shown above to import multiple images give an intriguing **Use Virtual Stack** option.
...
>
>
> ![](assets/1.6/imagej-files-virtual-stack.png)
>
>
> Still, virtual stacks are very handy if you want to peak into a large image without opening it all.
> If you don't mind its limitations, you can also open an ImageJ TIFF as a virtual stack using **File → Import → TIFF Virtual Stack...**.

### Saving images for analysis

To save an image, **File → Save As → Tiff...** *might* be all you ever need.
...
One way to mitigate this is to choose **File → Save As → ZIP...**.
...
Alternatively, I regularly use **File → Save As... → PNG...** as my preferred option to save RGB images for display using lossless compression.

If file size is still an issue, and artefacts are acceptable, then **File → Save As... → Jpeg...** can be used instead.
The quality of the JPEG can be adjusted via **Edit → Options → Input/Output...**: increasing the JPEG quality will give (slightly) larger files, but less obvious artefacts.

![](assets/1.6/imagej-files-options.png)
...
![](assets/1.6/imagej-files-bioformats.png)
...
![](assets/1.6/imagej-files-bioformats-series.png)
...
To save a file, you can use **Plugins → Bio-Formats → Bio-Formats Exporter**.
...
If this happens, then you can call on **Plugins → Bio-Formats → Bio-Formats Importer**  to force the import to use Bio-Formats, in the hope it will have more success.
...
![](assets/1.6/imagej-files-wsi-error.png)
...
> It can still be read using the **File → Import → Raw...** command, but to do so you will need to figure out the necessary metadata and input the appropriate values.
>
...
>
> __Note:__ The option **Little-endian byte order** relates to whether the bytes of 16 or 32-bit images are arranged from the least-to-most significant, or most-to-least significant.
