...
As described in chap_pixels, each pixel has a numerical value – but a pixel cannot typically have just *any* numerical value it likes.
...
![Bob devising his code.](assets/1.3/bobs-code.png)

*Figure —* Bob devising his code.
...
![Representing an image using different bit-depths.](assets/1.3/gen_fig-bit-depths-demo.png)

*Figure —* Representing an image using different bit-depths.

As shown in the figure, a 1-bit image can contain only two values: here, shown as black or white pixels.
...
>
> You may recall how the figure showed an image as being a matrix of numbers.
...
The main point of chap_pixels is that we need to
...
Of the two problems, clipping is usually the more serious, as shown in the figure.
...
Rounding can be a bigger problem when it comes to processing operations like filtering, which often involve computing averages over many pixels (see chap_filters).
