# ilastik — UI Workflow: Run a Pre-trained Pixel Classification Project

Use this workflow when you already have a trained ilastik pixel-classification
project and want to apply it to an image from Fiji.

## Before You Start

- Enable the `ilastik` update site in Fiji and restart.
- Have a trained `.ilp` project file ready.
...
1. Open `Plugins > ilastik > Run Pixel Classification Prediction`.
...
1. Choose `Probabilities` when you need per-class probability maps.
2. Choose `Segmentation` when you need one label image directly from ilastik.
...
1. For probability output, inspect the multi-channel float result and threshold
   it downstream if needed.
...
- If you are preparing data for ilastik training, export it from Fiji with
  `Export HDF5` first.
- For headless execution from Fiji scripting, use
  `GROOVY_WORKFLOW_PIXEL_CLASSIFICATION.groovy`.
